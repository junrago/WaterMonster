...
const BottleBar1000Svg = ({ fillRatio = 0.5 }) => {
  // 基本ボトル座標
  const bodyLeftX = 27;
  ...
  const baseAmp = 1;    // 通常波の高さ
  const bounceAmp = 8; // バウンス最大
  const waveFreq = 0.7;
  const N = 24;         // 波の分割数
  ...
  // --- 水パス ---
  let waterPath = '';
  if (fillRatio > 0) { // fillRatio=0で水非表示
      ...
      <G>
        ...
        <Path
          d={`M ${bodyLeftX},${bodyBotY - 10}
            ...
            L ${neckRightX},${bodyTopY}
            L ${neckLeftX},${bodyTopY}
...
export default BottleBar1000Svg;
